...
Coder 起動後、リポジトリルートで以下を 1 コマンド実行するだけで開発準備が完了する。
...
|----------|------|
| 1 | PostgreSQL の起動確認 |
| 2 | Valkey（Redis）の起動確認 |
| 3 | `apps/api/.env` の作成（未作成時のみ `.env.example` からコピー） |
| 4 | DB マイグレーションの適用 |
| 5 | フロントエンド依存関係のインストール（`node_modules` 未作成時のみ） |

各ステップは `✓ 完了` / `⚠ スキップ` / `✗ エラー` で結果を表示する。  
**再実行しても安全** — 既に完了済みのステップ（`.env` 作成済み・`node_modules` 存在など）はスキップされ、高速に終了する。

完了後は「起動手順（日常的な開発）」のコマンドでバックエンド・フロントエンドを起動する。
